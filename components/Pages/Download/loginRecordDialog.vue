  ...
  >
    ...
    <div class="list">
      ...
      <div class="block single-month" v-show="!isInterval">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm">
          <span class="demonstration">登入記錄月份：</span>
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button
        ...
        @click="
          !isInterval
            ? downloadLoginRecord('ruleForm')
            : downloadLoginRecord('ruleForm2')
        "
        >下載</el-button
      >
    </span>
...
export default {
  ...
  methods: {
    downloadLoginRecord(formName) {
      const vm = this;

      vm.$refs[formName].validate((valid) => {
        if (valid) {
          if (formName === 'ruleForm') {
            vm.startDate = dayjs(vm.ruleForm.month1).format('YYYY/MM');
            vm.endDate = dayjs(vm.ruleForm.month1).format('YYYY/MM');

            vm.getLoginRecordExcel();
          } else if (formName === 'ruleForm2') {
            vm.startDate = dayjs(vm.ruleForm2.month2).format('YYYY/MM');
            vm.endDate = dayjs(vm.ruleForm2.month3).format('YYYY/MM');

            vm.getLoginRecordExcel('range');
          }
          vm.resetForm();
        }
      });
    ...
    async getLoginRecordExcel(val) {
      ...
      const res = await backendApi.getLoginRecordExcel(data);
      if (res) {
        try {
          ...
          if (val === 'range') {
            vm.name = `${vm.startDate}-${vm.endDate}`;
          } else {
            vm.name = vm.startDate;
          }
          const a = document.createElement('a');
          const blob = new Blob([res], { type: 'application/excel' });
          a.href = window.URL.createObjectURL(blob);
          a.download = `${vm.name}登入記錄統計.xlsx`;
